...
- **Notification test trigger endpoint**: `POST /api/notifications/test-notification` is available only in Development and also requires authenticated `Admin` authorization.
...
- When `Jwt:ExpirationMinutes` is present but invalid (non-numeric or <= 0), the API logs a `Warning` and falls back to the 15-minute default.
